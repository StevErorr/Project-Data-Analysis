#dash versi : 2.18.2
#streamlit run dashboard/dashboard.py CARA ME RUN DI ANACONDA PROMPT

#background-color: #87CEFA !important; /* Light Sky Blue */
        #/* background-color: #64CCC5 !important;*/ /* Coastal Blue */
...
try:
    ...
    except (KeyError, ValueError): # Tangkap KeyError jika kolom tidak ada dan ValueError jika format salah
        st.warning("Kolom 'DAY_dteday' tidak ditemukan atau formatnya tidak valid. Melewati konversi untuk kolom ini.")
        #Opsi jika kolom tidak ada maka buat kolom kosong
        #df['DAY_dteday'] = pd.NaT
...
# Mengganti nama kolom (menghapus awalan DAY_) kecuali untuk kolom DAY_dteday
# new_columns = {col: col.replace("DAY_", "") for col in df.columns if col.startswith("DAY_")}
# df = df.rename(columns=new_columns)

new_columns = {col: col.replace("DAY_", "") for col in df.columns if col.startswith("DAY_") and col != "DAY_dteday"}
df = df.rename(columns=new_columns)

# Menampilkan nama-nama kolom dan tipe datanya untuk cek
# st.write("Nama-nama kolom dalam DataFrame:", df.columns)
...
def create_box_plot(df, x_col, y_col, title):
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.boxplot(x=x_col, y=y_col, data=df, ax=ax) #gunakan ax=ax
    ax.set_title(title)
    ax.set_xlabel(x_col)
    ax.set_ylabel('Jumlah ' + y_col) # Label y yang lebih deskriptif
    plt.tight_layout()
    return fig
...
# 4. Membuat heatmap
def create_heatmap(df): # Hanya menerima df sebagai argumen
    try:
...
def create_line_chart_2(df, x_col, y_col, title):
    fig, ax = plt.subplots(figsize=(12, 8)) # Perbaikan: simpan figure dan axes ke variabel fig dan ax
    hourly_avg = df.groupby('HOUR_hr')[[x_col, y_col]].mean() # Gunakan x_col dan y_col sebagai parameter
    ax.plot(hourly_avg.index, hourly_avg[x_col], label='Kasual', marker='o', color='lightcoral') # Gunakan ax untuk memplot
    ax.plot(hourly_avg.index, hourly_avg[y_col], label='Terdaftar', marker='o', color='lightskyblue') # Gunakan ax untuk memplot
    ...
    ax.legend()
    ax.grid(True)
    fig.tight_layout() # Gunakan fig untuk tight_layout
    return fig # Sekarang fig sudah didefinisikan dengan benar
...
def create_pie_chart_2(df, x_col, y_col, title):
    ...
    fig, ax = plt.subplots(figsize=(3, 2.8)) #membuat figure dan ax

    if total == 0:
        print("Tidak ada data penyewaan untuk dihitung proporsinya.")
        ax.pie([1, 0], labels=["Tidak ada penyewaan", ""], autopct='', startangle=90) #menggunakan ax untuk pie
        ax.set_title('Proporsi Penyewaan Kasual vs Terdaftar') #menggunakan ax untuk title
    else:
        ...
        ax.set_title('Proporsi Penyewaan Kasual vs Terdaftar Per Jam') #menggunakan ax untuk title
        ax.axis('equal') #menggunakan ax untuk axis
    fig.tight_layout() #menggunakan fig untuk tight_layout
    return fig
...
def create_bar_chart_2(df, x_col, y_col, title):
    ...
    plt.figure(figsize=(12, 8))
    x = np.arange(len(hourly_avg))  # Sekarang np sudah didefinisikan
    width = 0.35
    ...
    plt.tight_layout()
    return plt.gcf() # Mengembalikan figure matplotlib
...
st.write(f"Tanggal : {now.strftime('%Y-%m-%d   Jam : %H:%M:%S')}")
...
# # Periksa apakah kolom 'tanggal' ada
# if 'tanggal' not in df.columns:
#     st.error("Kolom 'tanggal' tidak ditemukan. Periksa nama kolom di file CSV.")
#     st.stop()
...
#     st.error(f"Kesalahan konversi tanggal: {e}. Pastikan format tanggal di CSV sudah benar.")
#     st.stop()
